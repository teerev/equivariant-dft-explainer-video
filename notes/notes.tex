...
\label{eq:continuous-eq5}
  &= \int_{-\infty}^{\infty}\!\!\int_{-\infty}^{\infty}
     X\bigl((s + \sigma) - a,\; (t + \tau) - b\bigr)\,
     F(\sigma,\tau)\,\mathrm{d}\sigma\,\mathrm{d}\tau
     &&\text{(rearranging terms inside the arguments).}
\end{align}
...
\subsection{ALL NEEDS REVISING FROM HERE: Rotational non-equivariance: continuous case}
...
(RX)(x) := X(R^{-1}x),
\end{equation}
so that $RX$ is just $X$ viewed in a rotated coordinate system.  Rotation
equivariance of the convolution would mean
\begin{equation}
\label{eq:rotation-equivariance}
R(X*F) \;=\; (RX)*F
\quad\text{for all }X\text{ and all rotations }R.
\end{equation}
...
\bigl(R(X*F)\bigr)(x)
  = (X*F)(R^{-1}x)
  = \int_{\mathbb{R}^2} X(R^{-1}x + y)\,F(y)\,dy.
\end{equation}
On the other hand,
\begin{equation}
\label{eq:rotation-right}
\bigl((RX)*F\bigr)(x)
  = \int_{\mathbb{R}^2} (RX)(x + y)\,F(y)\,dy
  = \int_{\mathbb{R}^2} X(R^{-1}(x + y))\,F(y)\,dy.
\end{equation}
...
\bigl((RX)*F\bigr)(x)
  = \int_{\mathbb{R}^2} X(R^{-1}x + z)\,F(Rz)\,dz.
\end{equation}

For rotation equivariance we would need, for all $X$ and $x$,
\begin{equation}
...
\int_{\mathbb{R}^2} X(R^{-1}x + z)\,F(Rz)\,dz.
\end{equation}
Since this must hold for every $X$, the integrands must agree almost
everywhere:
\begin{equation}
...
\end{equation}
Thus $F$ must be invariant under every rotation $R$, i.e.
\begin{equation}
\label{eq:kernel-invariance}
F(Ru) = F(u)\quad\forall\,u,\,R\in SO(2),
\end{equation}
which means that $F$ is rotationally symmetric (radial, $F(u)=f(\|u\|)$).
