...
Formalising it a bit, the convolutional layer depends on four functions $X$ (the input image), $F$ (the kernel), $Y$ (the feature map), and $f$, the function that keeps track of the indices between the input image and the feature map.
...
The +2 and +1 are there because the kernel is shifted across 2 pixels and up 1 pixel, and this corresponds directly to the position of the indices i and j in the feature map pointing to the output pixel, so we can identify the 2 and the 1 and i and j in the feature map coordinates:
...
\subsection{ALL NEEDS REVISING FROM HERE: Translational equivariance: discrete case}

If you translate the input image $F$ according to some translation $g$ and then perform the convolution $f$, the resulting feature map is the same as if you do the convolution $f$ first and then translate the feature map by the same translation $g$. Here, $g \in T(\mathbb{Z}^2)$, where $T(\mathbb{Z}^2)$ denotes the group of discrete translations on the pixel grid.

Here is the proof:

Let $g$ be a translation by $(a, b) \in \mathbb{Z}^2$, so that $(g F)(i', j') = F(i' - a, j' - b)$ for any $(i', j') \in \mathbb{Z}^2$.

We need to show that $(g F) * W = g (F * W)$, i.e., that translating the input first and then convolving yields the same result as convolving first and then translating the feature map.

Consider the left-hand side: applying translation first, then convolution. At feature map position $(i, j)$:

\begin{equation}
\label{eq:discrete-eq1}
((g F) * W)_{i j} = \sum_{u = 0}^{L-1} \sum_{v = 0}^{K-1} (g F)_{\,i - u,\ j - v}\, W_{u v}
\end{equation}

Since $(g F)_{i - u, j - v} = F((i - u) - a, (j - v) - b) = F(i - u - a, j - v - b)$, we have:

\begin{equation}
\label{eq:discrete-eq2}
((g F) * W)_{i j} = \sum_{u = 0}^{L-1} \sum_{v = 0}^{K-1} F_{\,i - u - a,\ j - v - b}\, W_{u v}
\end{equation}

Now applying convolution first, then translation gives:

\begin{equation}
\label{eq:discrete-eq3}
(g (F * W))_{i j} = (F * W)_{i - a, j - b} = \sum_{u = 0}^{L-1} \sum_{v = 0}^{K-1} F_{\,i - a - u,\ j - b - v}\, W_{u v}
\end{equation}

And immediately we see that $((g F) * W)_{i j} = (g (F * W))_{i j}$ for all $(i, j)$, establishing translational equivariance.
