...
class ConvolutionRadialHarmonicsVis(RightRegionScene):
    def construct(self):
        ...
        kernel_span = 3.0
        basis_images, basis_zeros = self.generate_radial_basis_images(
            num_basis=6, resolution=240, span=kernel_span
        )
        kernel_width = axes.width * 0.95
        ...
        kernel_size = min(basis_images[0].width, basis_images[0].height) * 0.39
        ...
        basis_labels = [
            self.create_basis_label(z, idx + 1) for idx, z in enumerate(basis_zeros)
        ]
    ...
    def generate_radial_basis_images(
        self,
        num_basis=6,
        resolution=220,
        span=6.0,
        ...
        effective_span = span * extent_scale
        xs = np.linspace(-effective_span, effective_span, resolution)
        ys = np.linspace(-effective_span, effective_span, resolution)
        grid_x, grid_y = np.meshgrid(xs, ys)
        radius = np.sqrt(grid_x**2 + grid_y**2)
        r_norm = np.clip(radius / effective_span, 0.0, 1.0)
        mask = radius <= effective_span

        falloff_start = np.clip(1.0 - edge_falloff, 0.0, 1.0)
        falloff_region = np.clip((r_norm - falloff_start) / edge_falloff, 0.0, 1.0)
        soft_mask = (1.0 - falloff_region) ** 2

        zeros = jn_zeros(0, num_basis)
        images = []
        ...
        for zero in zeros:
            ...
            rgba[..., 0] = radial
            rgba[..., 3] = np.clip(radial * 1.4, 0, 1) * soft_mask
    ...
    def create_basis_label(self, zero, index):
        label = MathTex(
            r"R_{%d}(r) = J_0(\alpha_{%d} \, r)" % (index, index),
            color=GREY_B,
        ).scale(0.7)
        return label
