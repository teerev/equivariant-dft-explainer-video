...
class ConvolutionAngularHarmonicsVis(RightRegionScene):
    def construct(self):
        ...
        basis_images = self.generate_angular_basis_images(
            angular_sequence=angular_sequence, resolution=240, span=kernel_span
        )
        kernel_width = axes.width * 1.0
        ...
        basis_labels = [
            self.create_angular_label(kind, order, idx + 1)
            for idx, (order, kind) in enumerate(angular_sequence)
        ]
    ...
    def generate_angular_basis_images(
        self,
        angular_sequence,
        resolution=220,
        span=3.0,
        ...
        for order, kind in angular_sequence:
            ...
            field = np.where(mask > 0.0, field, 0.0)
            ...
            rgba[..., 0] = field
            rgba[..., 3] = np.clip(field * 1.4, 0, 1) * mask
    ...
    def create_angular_label(self, kind, order, index):
        if kind == "sin":
            expr = r"\sin(\mu_{%d} \theta)" % index
        else:
            expr = r"\cos(\mu_{%d} \theta)" % index
        label = MathTex(
            r"A_{%d}(\theta) = %s" % (index, expr),
            color=GREY_B,
        ).scale(0.7)
        return label
